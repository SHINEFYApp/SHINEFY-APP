            ...
            <View className="flex-row flex-wrap justify-between">
              <FlatList
                ...
                renderItem={({index , item:ele})=>{
                  return <SelectCardDetails
                    onPress={()=>{setCurrentActive(index)}}
                    active={currentActive}
                    id={index}
                    ele={ele}
                    keyObj={title.includes('Model')? "modal" :handleTitleData()}
                    key={index}
                    title={title.includes('Model') ? ele.modal[0] :ele[handleTitleData()][0]}
                    color={ele.color_code}
                    img={title.includes('Model') ? ele.modal[0] : ele.image ? ele.image : ele[`${handleTitleData()}_image`]}
                    />
                }}
              />
