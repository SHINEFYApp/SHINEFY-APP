...
import {View,Image,BackHandler,Keyboard,Text,Modal,FlatList,StyleSheet,TextInput,StatusBar,TouchableOpacity,SafeAreaView,ImageBackground,Dimensions, Platform} from "react-native";
...
export default class Change_password extends Component {
    ...
    _updatePassword = async () => {
        Keyboard.dismiss()
        let {old_password,con_password,new_password} = this.state;
        ...
        apifuntion.postApi(url,data).then((obj) => {
            if(obj.success == 'true'){
                ...
                var userValue = {name:user_arr.name,phone_number:user_arr.phone_number,password:new_password,user_id:user_arr.user_id,login_type:'app',user_type:1};
                        ...
                        <View style={Styles.container}>
                            ...
                            <ImageBackground source={localimag.new_header} style={{width:mobileW*100/100,height:mobileW*20/100}} >
                                <View style={{width:mobileW*100/100,justifyContent:'center',alignSelf:'center',alignItems:'center',flexDirection:'row',paddingVertical:mobileW*6/100}}>
                                    <TouchableOpacity activeOpacity={0.7} style={{justifyContent:'center',width:'20%',alignItems:'center'}}
                                    ...
                                    <View style={{width:'80%',alignItems:'center',paddingRight:mobileW*13/100}}>
                                        <Text style={{fontFamily:Font.fontsemibold,fontSize:mobileW*5.5/100,color:Colors.whiteColor}}>{Lang_chg.changepassword_txt[config.language]}</Text>
                                    </View>
                                </View>
                            </ImageBackground>
                            
                            <View style={{flexDirection:'row',marginTop:mobileW*10/100,width:mobileW*85/100,alignSelf:'center',alignItems:'center',borderBottomWidth:1,borderBottomColor:Colors.pass_bottom_border}}>
                                    <View style={{width:'10%'}}>
                                    <Image source={localimag.padlock1} resizeMode='contain' style={{height:mobileW*4.5/100,width:mobileW*4.5/100,paddingLeft:mobileW*5/100}}/>
                                </View>
                                <TextInput 
                                ...
                                />
                                <TouchableOpacity activeOpacity={0.7} onPress={() => {this.setState({securepass1: !this.state.securepass1})}} style={{width:'15%'}}>
                                    {this.state.securepass1 == true ? 
                                        <Text style={{color:Colors.black_color,fontFamily:Font.fontmedium, fontSize:mobileW*3.4/100}}>{Lang_chg.show[config.language]}</Text>
                                    :
                                        <Text style={{color:Colors.black_color,fontFamily:Font.fontmedium, fontSize:mobileW*3.4/100}}>{Lang_chg.Hide[config.language]}</Text> 
                                    }
                                </TouchableOpacity>
                            </View>
                            <View style={{flexDirection:'row',marginTop:mobileW*6/100,width:mobileW*85/100,alignSelf:'center',alignItems:'center',borderBottomWidth:1,borderBottomColor:Colors.pass_bottom_border}}>
                                    <View style={{width:'10%'}}>
                                    <Image source={localimag.padlock1} resizeMode='contain' style={{height:mobileW*4.5/100,width:mobileW*4.5/100,paddingLeft:mobileW*5/100}}/>
                                </View>
                                <TextInput 
                                ...
                                />
                                <TouchableOpacity activeOpacity={0.7} onPress={() => {this.setState({securepass2: !this.state.securepass2})}} style={{width:'15%'}}>
                            ...
                            <View style={{flexDirection:'row',marginTop:mobileW*6/100,width:mobileW*85/100,alignSelf:'center',alignItems:'center',borderBottomWidth:1,borderBottomColor:Colors.pass_bottom_border}}>
                                    ...
                                    <TextInput 
                                        ...
                                        onSubmitEditing={() => {Keyboard.dismiss()}}
                                        autoCompleteType='off'
                                        onChangeText={(txt) => { this.setState({ con_password: txt }) }}
                                    />
                                    <TouchableOpacity activeOpacity={0.7} onPress={() => {this.setState({securepass3: !this.state.securepass3})}} style={{width:'15%'}}>
                            ...
                            <TouchableOpacity activeOpacity={0.7} style={{backgroundColor:Colors.appColor,width:mobileW*80/100,borderRadius:25,marginTop:mobileH*5/100,alignSelf:'center'}}
                                onPress={()=>{this._updatePassword()}}>
                                <Text style={{color:Colors.whiteColor, alignSelf:'center' ,fontSize:mobileW*4/100,fontFamily:Font.fontmedium,paddingVertical:mobileW*2/100}}>{Lang_chg.resetpassword_txt[config.language]}</Text>
                            </TouchableOpacity>
                        </View>
                        <HideWithKeyboard>
                            <ImageBackground source={localimag.bottom_logo} style={{height:mobileW*65/100,width:mobileW*100/100,bottom:0}}/>
                        </HideWithKeyboard>
            </View>
        );
    }
