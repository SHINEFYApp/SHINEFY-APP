...
class NotificationProvider {
  ...
  notification_arr_schedule(notification_arr) {
    ...
    for (let i = 0; i < notification_arr.length; i++) {
      ...
      this.notificationfirebaseDriversend(message, action_json, player_id, title)
    }
  }
  ...
  notificationfirebasesend = (massege, action_json, playerid, title) => {
    let headers = new Headers({
      "Content-Type": "application/json",
      Authorization: "key=AAAA2A_OxE4:APA91bHeV1n1vM62ONqy1w4VvfH0ntmONK6YIFGQdw1hSTK1awNbvQpkiP8rffcg55jV5dVTIM_-kRJ3VGc6PQB5RcWEfvJOggyXaN2vgiQ-UWAMPNPi5V4nX7RPJbqpfaa2BY3OTIaJ",
    })
    consolepro.consolelog('action_json', action_json)
  ...
  notificationfirebaseDriversend = (massege, action_json, playerid, title) => {
    let headers = new Headers({
      "Content-Type": "application/json",
      Authorization: "key=AAAAgvQiwnI:APA91bF-qqeFiMxlqUqlFhlFPAp8iJpFD2KEzTe-VWSGIZ5o-Sb2GfOYUXWK9c9Xe04IWlNDo1HF9huLeE-0L0-FSq_aeSx3K5EIxkfWNy4HLCrEP_FxOlbL-zjlghUSoyb9U94YpZKe",
    })
    consolepro.consolelog('action_json', action_json)
    consolepro.consolelog('action_json', massege)
    consolepro.consolelog('action_json', title)
