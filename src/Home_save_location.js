...
export default class Home_save_location extends Component {
  ...
  callLocation = async that => {
    localStorage.getItemObject('positions').then(positions => {
      if (positions !== null) {
        var pointcheck1 = 0;
        this.getalldata(positions);
  ...
  getadddressfromlatlong = async event => {
    ...
    for (let i = 0; i < responseJson.address_components.length; i++) {
      if (responseJson.address_components[i].types[0] === 'locality') {
        responseJson.address_components[i].long_name;
        break;
      } else if (
  ...
  render() {
    return (
      <View style={styles.container}>
        <SafeAreaView style={{backgroundColor: Colors.theme_color, flex: 1}} />
        <StatusBar
